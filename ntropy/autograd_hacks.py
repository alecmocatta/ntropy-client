...
def compute_grad1(model: nn.Module, loss_type: str = 'mean') -> None:
    ...
    for layer in model.modules():
        ...
        if layer_type == 'Linear':
            ...
            setattr(layer.weight, 'grad1normsq', torch.einsum('ni,ni,nj,nj->n', B, B, A, A))
            setattr(layer.weight, 'grad1weighted', lambda x, A=A, B=B: torch.einsum('n,ni,nj->ij', x, B, A))
            if layer.bias is not None:
                # setattr(layer.bias, 'grad1', B)
                setattr(layer.bias, 'grad1normsq', torch.einsum('ni,ni->n', B, B))
                setattr(layer.bias, 'grad1weighted', lambda x, A=A, B=B: torch.einsum('n,ni->i', x, B))
